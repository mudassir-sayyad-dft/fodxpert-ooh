...
abstract class FFmpegVideoEditorConfig {
  ...
  const FFmpegVideoEditorConfig(this.controller,
      {this.name,
  ...
  String get cropCmd {
    if (controller.minCrop <= Offset.zero &&
        controller.maxCrop >= Offset(1.0, 1.0)) {
      return "";
    }
...
class VideoFFmpegVideoEditorConfig extends FFmpegVideoEditorConfig {
  ...
  Future<FFmpegVideoEditorExecute> getExecuteConfig() async {
    ...
    String thumbImg = "";
    if (controller.selectedCoverVal != null &&
        controller.selectedCoverVal!.thumbData != null) {
      final String tempPath =
          outputDirectory ?? (await getTemporaryDirectory()).path;
      final String n = name ?? path.basenameWithoutExtension(videoPath);
      final int epoch = DateTime.now().millisecondsSinceEpoch;

      thumbImg = "$tempPath/${n}_$epoch.jpg";
      File(thumbImg).writeAsBytes(controller.selectedCoverVal!.thumbData!);
    }
    ...
    print(
        "$startTrimCmd -i \'$videoPath\' ${thumbImg.isEmpty ? "" : "-i \'$thumbImg\' -map 0 -map 1 -c copy -disposition:v:1 attached_pic"} $toTrimCmd ${filtersCmd(filters)} $gifCmd ${filters.isEmpty ? '-c copy' : ''} -y \'$outputPath\'");
