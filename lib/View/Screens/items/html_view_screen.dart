...
import 'package:fodex_new/View/Components/dialogs/edit_template_info_dialog.dart';
...
class HtmlViewScreen extends StatefulWidget {
  ...
  const HtmlViewScreen({super.key, required this.template});
...
class _HtmlViewScreenState extends State<HtmlViewScreen> {
  readTeampleFromLocalStorage() async {
    ...
    final file = File(
        '${downloadDirectory.path}/fodx/templates/${widget.template.id}/${widget.template.id}/index.html');
  ...
  loadData() async {
      ...
      if (data.type == TemplateDataType.IMAGE) {
        if (data.value.startsWith("https") || data.value.startsWith("http")) {
          ...
          final bytes = await File(
                  "${downloadDirectory.path}/fodx/templates/${widget.template.id}/${widget.template.id}/${data.value}")
              .readAsBytes();
    ...
    await webViewController.setOnConsoleMessage((var message) {
  ...
  onSave(var d, var message) async {
    {
      jsonFile.value[message.message]['data'] =
          d.map((e) => e.toMap()).toList();
      ...
      for (int i = 0; i < d.length; i++) {
        ...
        if (data2.type == TemplateDataType.IMAGE) {
          if (data2.value.startsWith("https") ||
            ...
            Directory downloadDirectory = await getTemporaryDirectory();
            final bytes = await File(
                    "${downloadDirectory.path}/fodx/templates/${widget.template.id}/${widget.template.id}/${data2.value}")
                .readAsBytes();
  ...
  showDataDialog(var message, var data) {
    showDialog(
        context: context,
        builder: (context) => EditPreviewDialog(
            temp: widget.template,
            onSave: (d) => onSave(d, message),
            title: data['title'],
  ...
  saveImageJson(String templateName) async {
    ...
    try {
      ...
      await zipTemplate(context, templateName, () async {
        ...
        final uploadId = await adsController.addNewAd(
          zipFile,
